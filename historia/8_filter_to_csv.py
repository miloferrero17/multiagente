...
def main():
    parser = argparse.ArgumentParser(
        description="Exportar metadata de Pinecone a CSV con paginación y filtro por cualquier campo."
    )
    parser.add_argument("--namespace", default="trello", help="Namespace de Pinecone.")
    parser.add_argument(
        "--output",
        default="",
        help="Nombre del CSV. Si se omite, se usa <filter_value>.csv.",
    )
    parser.add_argument(
        "--filter_field",
        required=True,
    ...
    # Archivo: si no se pasa --output => <filter_value>.csv
    default_name = f"{slugify(args.filter_value)}.csv"
    ...
    # 3) Escribir CSV
    print(f"Escribiendo CSV en: {out_file}")
    ...
    print(f"✔ Exportación finalizada. Filas escritas: {len(rows)}")
