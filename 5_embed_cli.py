...
- --format pinecone => imprime sólo un resumen (NO imprime los floats).
...
def parse_args():
    ...
    p.add_argument("--format", choices=["default", "pinecone"], default="default",
                   help="'pinecone' imprime un resumen (NO imprime floats).")
...
def main():
    ...
    # Construyo 'out' completo (incluye embedding) pero NO lo voy a imprimir con los floats
    out = {"model": model, "input_length": len(text), "dim": len(emb) if hasattr(emb, "__len__") else None, "embedding": emb}

    # Si pidieron guardar full out en --out, lo hago (incluye embedding)
    if args.out:
        try:
    ...
    # Preparar salida pública (SIN embedding) para imprimir por consola
    out_public = {"model": out["model"], "input_length": out["input_length"], "dim": out["dim"]}

    # formato pinecone (no imprimir floats: mostramos resumen con la dimensión)
    if args.format == "pinecone":
        # impresión reducida (sin el array de floats)
        if args.pretty:
            print(json.dumps({"pinecone_vector": "omitted", "dim": out_public["dim"]}, ensure_ascii=False, indent=2))
        else:
            print(json.dumps({"pinecone_vector": "omitted", "dim": out_public["dim"]}, ensure_ascii=False))
        return

    # salida por defecto (json con embedding OMITIDO)
    if args.pretty:
        print(json.dumps(out_public, ensure_ascii=False, indent=2))
    else:
        print(json.dumps(out_public, ensure_ascii=False))
