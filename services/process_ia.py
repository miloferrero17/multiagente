#!/usr/bin/env python3
"""
services/process_ia.py

Agrega/normaliza todos los resultados JSON en out/*.json en un único archivo listo para
vectorización.

Características:
- lee todos los archivos JSON en --out-dir (por defecto "out")
- soporta salida JSON (array) o NDJSON (1 objeto por línea)
- opción --include-source para añadir el campo "_source_file" a cada objeto
- escritura atómica (escribe a .tmp y luego renombra)
- ignora archivos inválidos y muestra resumen al final

Uso (desde la raíz del repo):
  python3 services/process_ia.py
  python3 services/process_ia.py --out-dir out --target ready_vectorizacion.json --format json --include-source
  python3 services/process_ia.py --out-dir out --target ready_vectorizacion.ndjson --format ndjson
"""
...
def collect_from_out(out_dir: str, include_source: bool) -> List[Dict[str, Any]]:
    ...
    for p in files:
        try:
            ...
            print(f"[WARN] Skipping {p} (json load error): {e}", file=sys.stderr)
            skipped += 1
            continue
        ...
        if isinstance(data, list):
            for el in data:
                if isinstance(el, dict):
                    if include_source:
                        el = dict(el)
                        el["_source_file"] = base
                    items.append(el)
                else:
                    wrapped = {"value": el}
                    if include_source:
                        wrapped["_source_file"] = base
                    items.append(wrapped)
        elif isinstance(data, dict):
            obj = dict(data)
            if include_source:
                obj["_source_file"] = base
            items.append(obj)
        else:
            wrapped = {"value": data}
            if include_source:
...
def write_json_atomic(target: str, items: List[Dict[str, Any]]) -> None:
    tmp = target + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(items, f, ensure_ascii=False, indent=2)
    os.replace(tmp, target)

def write_ndjson_atomic(target: str, items: List[Dict[str, Any]]) -> None:
    tmp = target + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        for obj in items:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="services/process_ia.py",
        description="Aggregate out/*.json into a single ready_vectorizacion file."
    )
    p.add_argument("--out-dir", default="out", help="Directory with per-comment JSON files (default: out)")
    p.add_argument("--target", default="ready_vectorizacion.json", help="Target output file (default: ready_vectorizacion.json)")
    p.add_argument("--format", choices=("json", "ndjson"), default="json", help="Output format: json (array) or ndjson (one obj per line)")
    p.add_argument("--include-source", action="store_true", help="Include _source_file field indicating origin filename")
    return p.parse_args()
...
def main() -> int:
    ...
    if not os.path.isdir(out_dir):
        print(f"[ERROR] out-dir '{out_dir}' does not exist or is not a directory.", file=sys.stderr)
        return 2

    items = collect_from_out(out_dir, include_source=include_source)
    total_files = len(sorted(glob.glob(os.path.join(out_dir, "*.json"))))
    print(f"[INFO] Collected {len(items)} items from {total_files} file(s) in '{out_dir}'")

    try:
        if fmt == "json":
            write_json_atomic(target, items)
        else:
            write_ndjson_atomic(target, items)
    except Exception as e:
        print(f"[ERROR] Failed writing target '{target}': {e}", file=sys.stderr)
        return 3

    print(f"[INFO] Wrote {len(items)} items to '{target}' (format={fmt})")
    return 0
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--delay", "-d", type=float, default=float(os.getenv("DELAY_SECS", "0.6")), help="Delay between calls in seconds")
